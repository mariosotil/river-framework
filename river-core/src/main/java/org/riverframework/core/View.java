...
public interface View extends Base {
	 ...
	 *
	 * @param <U>
	 *            a class that inherits from AbstractDocument and implements Document.
	 * @param clazz
	 *            a class object that inherits from AbstractDocument and implements Document.
	 * @param key
	 *            the document's key
	 ...
	 * 
	 * @param query
	 *            the query in the wrapper's format
	 ...
	 * Searches by the query and returns a DocumentList object with the documents that match. So far, The style of the
	 * query will depend on how is implemented in the wrapper loaded. For example, with the lotus.domino wrapper, it
	 * will be something like "Black AND Dog". The max parameter will indicate how much documents could retrieve the
	 * method. It will depend on what wrapper is being used.
	 * 
	 * @param query
	 *            the query in the wrapper's format
	 * @param max

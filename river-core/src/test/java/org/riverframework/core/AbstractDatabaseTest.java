...
public abstract class AbstractDatabaseTest {
	...
	static class VacationDatabase extends
			AbstractIndexedDatabase<VacationDatabase> {

		protected VacationDatabase(Session session,
				org.riverframework.wrapper.Database<?> _database) {
			super(session, _database);

			registerDocumentClass(VacationRequest.class);
	...
	static class VacationRequest extends AbstractDocument<VacationRequest> {
		...
		protected VacationRequest(Database database,
	...
	static class Person extends AbstractIndexedDocument<Person> {
		...
		protected Person(Database database,
				org.riverframework.wrapper.Document<?> _doc) {
			super(database, _doc);
		}
	...
	public void open() {
		...
		try {
			if (context == null) {
				...
				vacationDatabase = session.getDatabase(VacationDatabase.class,
	...
	public void testOpenedDatabase() {
		...
		assertFalse("The file path could not be detected.", database
				.getFilePath().equals(""));
		assertFalse("The database name could not be detected.", database
				.getName().equals(""));
	}
	...
	public void testCreateAndGetView() {
		...
		assertTrue(
				"There is a problem creating the view in the test database.",
				view.isOpen());
		...
		int i = 0;
		for (i = 0; i < 10; i++) {
			database.createDocument().setField("Form", form)
					.setField("Value", i).save();
		}

		view = database.getView(name);
		assertTrue(
				"There is a problem opening the last view created in the test database.",
				view.isOpen());
		...
		assertTrue(
				"There is a problem with the documents indexed in the last view.",
				i == 10);
		...
		assertTrue(
				"There is a problem with the last documents created when we try to delete them.",
				i == 0);
	...
	public void testSearch() {
		...
		database.createDocument().setField("Form", TEST_FORM)
				.setField("Value", "THIS_IS_THE_DOC").save();
		...
		assertTrue(
				"The search returns values for a query that would returns nothing.",
				!col.hasNext());

		col = null;
		col = database.search("THIS_IS_THE_DOC");
		assertTrue(
				"The search does not returns values for a query that would returns something.",
				col.hasNext());
	}
	...
	public void testGetDocument() {
		...
		temp = p.isOpen();
		assertTrue("It could not possible load the person object for Jake.",
				temp);
		...
		temp = p.isOpen();
		assertTrue("It could not possible load the person object for John.",
				temp);
		...
		temp = p.isOpen();
		assertTrue("It could not possible load the person object for Kathy.",
				temp);
		...
		assertFalse(
				"It should not be possible to load a person object for an unknown id.",
				p.isOpen());
	}
	...
	public void testGetFields() {
		...
		String id = database.createDocument().setField("ca_pe_name", "Kathy")
				.setField("Form", "fo_ap_people").setField("Age", 25).save()
				.getObjectId();
		...
		assertTrue(
				"It could not possible get the fields from the Kathy's document.",
				fields.size() > 0);
		...
		assertTrue(
				"It could not possible get the field Name from the Kathy's document.",
				value.equals("Kathy"));
		...
		assertTrue(
				"It could not possible get the field Age from the Kathy's document.",
				age == 25);
	...
	public void testOpenedExtendedDatabase() {
		assertTrue("The test database could not be opened.",
				vacationDatabase.isOpen());
		assertFalse("The file path could not be detected.", vacationDatabase
				.getFilePath().equals(""));
		assertFalse("The database name could not be detected.",
				vacationDatabase.getName().equals(""));
	}

	@Test
	public void testCreateAndGetExtendedView() {
		assertTrue("The test database could not be instantiated.",
				vacationDatabase != null);
		assertTrue("The test database could not be opened.",
				vacationDatabase.isOpen());
		...
		int i = 0;
		for (i = 0; i < 10; i++) {
			vacationDatabase.createDocument().setField("Form", form)
					.setField("Value", i).save();
		}
	...
	public void testCreateAndGetVacationRequest() {
		assertTrue(
				"The test database could not be opened as a VacationDatabase.",
				vacationDatabase.isOpen());

		VacationRequest doc = vacationDatabase
				.createDocument(VacationRequest.class);

		assertTrue(
				"There is a problem creating a new VacationRequest in the test database.",
				doc.isOpen());
		...
		assertTrue(
				"There is a problem getting a VacationRequest created in the test database.",
				doc.isOpen());
	}
	...
	public void testExtendedSearch() {
		...
		for (int i = 0; i < 10; i++) {
			vacationDatabase.createDocument().setField("Form", TEST_FORM)
					.setField("Value", rs.nextString()).save();
		}
	...
	public void testGetExtendedDocumentIterator() {
		assertTrue("The vacation database could not be instantiated.",
				vacationDatabase != null);
		assertTrue("The vacation database could not be opened.",
				vacationDatabase.isOpen());

		vacationDatabase.getAllDocuments().deleteAll();

		vacationDatabase.createDocument(VacationRequest.class)
				.setField("Requester", "John").setField("Time", 30).save();

		vacationDatabase.createDocument(VacationRequest.class)
				.setField("Requester", "Kathy").setField("Time", 25).save();

		vacationDatabase.createDocument(VacationRequest.class)
				.setField("Requester", "Michael").setField("Time", 27).save();

		DocumentIterator it = vacationDatabase.getAllDocuments();

		for (Document doc : it) {
			assertTrue(
					"It could not possible load the vacation request object from the DocumentIterator.",
					doc.isOpen());

			// Map<String, Field> fields = doc.getFields();
			// System.out.println(fields.size());
			assertTrue(
					"The vacation request object from the DocumentIterator is an instance from "
							+ doc.getClass().getName()
							+ ", and not from VacationRequest.", doc.getClass()
							.getSimpleName().contains("VacationRequest"));
		}
	}
	...
	public void testGetExtendedDocument() {
		...
		assertFalse("The database still has documents.", vacationDatabase
				.getAllDocuments().hasNext());

		vacationDatabase.createDocument(Person.class).setId("John")
				.setField("Age", 30).save();

		vacationDatabase.createDocument(Person.class).setId("Jake")
				.setField("Age", 27).save();

		vacationDatabase.createDocument(Person.class).setId("Kathy")
				.setField("Age", 25).save();

		Person p = vacationDatabase.getDocument(Person.class, "Kathy");
		assertTrue("It could not possible load the person object for Kathy.",
				p.isOpen());
		assertTrue("It could not possible get the Kathy's age.",
				p.getFieldAsInteger("Age") == 25);

		p = vacationDatabase.getDocument(Person.class, "Jake");
		assertTrue("It could not possible load the person object for Jake.",
				p.isOpen());
		assertTrue("It could not possible get the Jake's age.",
				p.getFieldAsInteger("Age") == 27);

		p = vacationDatabase.getDocument(Person.class, "John");
		assertTrue("It could not possible load the person object for John.",
				p.isOpen());
		assertTrue("It could not possible get the John's age.",
				p.getFieldAsInteger("Age") == 30);

		String unid = p.getObjectId();
		p = null;
		p = vacationDatabase.getDocument(Person.class, unid);
		assertTrue(
				"It should be possible to load a person object for Kathy with its Object Id.",
				p.isOpen());

		p = null;
		Document d = vacationDatabase.getDocument(DefaultDocument.class,
				"Kathy");
		assertFalse(
				"It should not be possible to load a person object for Kathy without indicate its class.",
				d.isOpen());

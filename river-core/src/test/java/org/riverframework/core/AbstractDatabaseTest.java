...
public abstract class AbstractDatabaseTest {
  ...
  final static String TEST_FORM = "TestForm";
  final static String TEST_VIEW = "TestView";
  final static String TEST_GRAPH = "TestGraph";
  ...
  public void open() {
    ...
    try {
      if (context == null) {
        ...
        vacationDatabase.getAllDocuments()
            .deleteAll();
  ...
  public void testOpenedDatabase() {
    ...
    assertFalse("The file path could not be detected.", database.getFilePath()
        .equals(""));
    assertFalse("The database name could not be detected.", database.getName()
        .equals(""));
  }
  ...
  public void testCreateAndGetView() {
    ...
    assertTrue("There is a problem opening the last view created in the test database.",
        view.isOpen());
    ...
    assertTrue("There is a problem with the last documents created when we try to delete them.",
        i == 0);
  ...
  public void testSearch() {
    ...
    assertTrue("The search does not returns values for a query that would returns something.",
        col.hasNext());
  }
  ...
  public void testGetDocument() {
    ...
    String unidJohn = database.createDocument()
        .setField("Name", "John")
        .setField("Age", 30)
        .save()
        .getObjectId();

    String unidKathy = database.createDocument()
        .setField("Name", "Kathy")
        .setField("Age", 25)
        .save()
        .getObjectId();

    String unidJake = database.createDocument()
        .setField("Name", "Jake")
        .setField("Age", 27)
        .save()
        .getObjectId();
  ...
  public void testGetFields() {
    ...
    Map<String, List<String>> fields = p.getFields();

    assertTrue("It could not possible get the fields from the Kathy's document.",
        fields.size() > 0);

    List<String> field = fields.get("ca_pe_name");
    String value = field.get(0)
        .toString();

    assertTrue("It could not possible get the field Name from the Kathy's document.",
        value.equals("Kathy"));

    // TODO: Define if we are going to use List<String> or List<Object>
    /*
...
*/
  ...
  public void testOpenedExtendedDatabase() {
    ...
    assertFalse("The file path could not be detected.", vacationDatabase.getFilePath()
        .equals(""));
    assertFalse("The database name could not be detected.", vacationDatabase.getName()
        .equals(""));
  }
  ...
  public void testCreateAndGetExtendedView() {
    ...
    int i = 0;
    for (i = 0; i < 10; i++) {
      vacationDatabase.createDocument()
          .setField("Form", form)
          .setField("Value", i)
          .save();
    }
  ...
  public void testGetExtendedDocumentIterator() {
    ...
    for (Document doc : it) {
      ...
      assertTrue("The vacation request object from the DocumentIterator is an instance from " + doc
          .getClass()
          .getName()
  ...
  public void testGetExtendedDocument() {
    ...
    assertFalse("The database still has documents.", vacationDatabase.getAllDocuments()
        .hasNext());
    ...
    assertTrue("It should be possible to load a person object for Kathy with its Object Id.",
        p.isOpen());

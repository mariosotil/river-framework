...
public abstract class AbstractDocumentTest {
  ...
  public void testCompareFieldValue() {
    ...
    doc.setField("FIELD_STRING", "SOME_TEXT")
        ...
        .setField("FIELD_OBJECT", new ArrayList<>(Arrays.asList(new String[]{"VAL1", "VAL2"})));
    ...
    assertTrue("The String array saved must be equal to the test value",
        doc.compareFieldValue("FIELD_STRING_ARRAY", new String[]{
            "VAL1", "VAL2"}));
    ...
    assertTrue("The Vector saved must be equal to the test value",
        doc.compareFieldValue("FIELD_OBJECT", new String[]{
            "VAL1", "VAL2"}));
...
*/
  ...
  public void testSetAndGetFieldAsArrayString() {
    ...
    List<String> newValue = doc.getField(testField);
  ...
  public void testIsModified() {
    ...
    doc.setField(testField, testValue);
    assertFalse(
        "A document's field was set with the same value, but now the document it's set as modified",
        doc.isModified());
  ...
  public void testHasField() {
    ...
    assertTrue("The document could not be created", doc.isOpen());
    assertFalse("The document is new and is set as a conflict.",
        doc.hasField("THIS_FIELD_DOES_NOT_EXIST"));

    doc.setField("Form", TEST_FORM);
    doc.setField("THIS_FIELD_DOES_NOT_EXIST", "");

    assertTrue("The document is a conflict and was not detected.",
        doc.hasField("THIS_FIELD_DOES_NOT_EXIST"));
  }

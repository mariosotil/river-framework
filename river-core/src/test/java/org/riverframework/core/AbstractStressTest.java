...
public abstract class AbstractStressTest {
  ...
  public void testStress1() {
    for (int depth = 0; depth <= testDepth; depth++) {
      ...
      Database library =
          session.createDatabase(Library.class, context.getTestDatabaseServer(),
              "TEST_LIBRARY_" + suffixDb);
      @SuppressWarnings("unused")
  ...
  public void testStress4() {
    for (int depth = 0; depth <= testDepth; depth++) {
      ...
      Database database =
          session
              .createDatabase(context.getTestDatabaseServer(), "TEST_DB_" + sdf.format(new Date())
                  + ".nsf");

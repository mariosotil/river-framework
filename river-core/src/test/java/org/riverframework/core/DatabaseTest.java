...
import org.riverframework.core.DefaultView;
...
public class DatabaseTest {
	...
	private DefaultDatabase database = null;
	private VacationDatabase vacationDatabase = null;
	...
	public void init() {
		...
		vacationDatabase = session.getDatabase(VacationDatabase.class, "", context.getDatabase());
	...
	public void testGetView() {
		...
		DefaultView rView = database.getView(DefaultView.class, TEST_VIEW);
	...
	static class VacationRequest extends DefaultDocument {
		...
		protected VacationRequest(Database d, org.riverframework.wrapper.Document doc) {
			super(d, doc);
		}
	...
	public void testCreateAndGetVacationRequest() {
		...
		VacationRequest rDocument = vacationDatabase.createDocument(VacationRequest.class);
		...
		rDocument = vacationDatabase.getDocument(VacationRequest.class, universalId);
	...
	public void testGetMainReplica() {
		...
		Database replica = database.getMainReplica();
	...
	public void testGetDocumentCollection() {
		...
		for (int i = 0; i < col.size(); i++) {
			VacationRequest v = (VacationRequest) col.get(i);
			assertTrue("It could not possible load the vacation request object from the DocumentCollection.", v.isOpen());
		}

		for (Document doc : col) {
			assertTrue("It could not possible load the vacation request object from the DocumentCollection.", doc.isOpen());
			assertTrue("The vacation request object from the DocumentCollection has not the right class.",

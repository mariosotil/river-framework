...
public class LoggerHelper {
  ...
  public LoggerHelper addFileHandler(String filename) {
    try {
      ...
      if (pos1 > -1 && pos2 > -1) {
        ...
        modifiedFilename =
            filename.substring(0, pos1) + date + filename.substring(pos2 + 2, filename.length());
      } else {
  ...
  public LoggerHelper setLevel(Level level) {
    ...
    try {
      ...
      log.warning("AccessControlException: it was not possible to set the log level. Message:" + e
          .getMessage());
    }

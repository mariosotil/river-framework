...
public abstract class AbstractFactory<N> implements org.riverframework.wrapper.Factory<N> {
  ...
  protected AbstractFactory(Class<? extends AbstractNativeReference<N>> nativeReferenceClass) {
    ...
    try {
      constructorNativeReference = nativeReferenceClass
          .getDeclaredConstructor(Base.class, ReferenceQueue.class);
  ...
  protected void registerReference(String id, Base<?> _wrapper) {
    try {
      AbstractNativeReference<N> ref = nativeReferenceClass
          .cast(constructorNativeReference.newInstance(_wrapper,
              wrapperQueue));
  ...
  protected <U extends Base<?>> U createWrapper(Class<U> outputClass, Class<? extends N> inputClass,
      N __obj) {
    U _wrapper = null;

    try {
      Constructor<?> constructor = outputClass
          .getDeclaredConstructor(org.riverframework.wrapper.Session.class,
              inputClass);
  ...
  protected <U extends Base<?>> U getWrapper(Class<U> outputClass, Class<? extends N> inputClass,
      String __obj) {
    U _wrapper = null;

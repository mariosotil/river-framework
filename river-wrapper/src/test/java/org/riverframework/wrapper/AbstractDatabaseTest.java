...
public abstract class AbstractDatabaseTest {
  ...
  public void testCreateAndGetView() {
    ...
    for (i = 0; i < 10; i++) {
      ...
      assertTrue(
          "There is a problem retrieving the Value field from the document with id '" + (i + 1)
    ...
    _it = _view.getAllDocuments();
    assertFalse("There is a problem with the last documents created when we try to delete them.",
        _it.hasNext());
  ...
  public void testSearch() {
    ...
    assertFalse("The search returns values for a query that would returns nothing.",
        _iterator.hasNext());
  ...
  public void testGetDocumentCollection() {
    ...
    for (Document<?> doc : _iterator) {
      assertTrue("It could not possible load the vacation request object from the DocumentList.",
          doc.isOpen());
    }
  }
}
